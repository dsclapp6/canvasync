...
| 3.12 | Category is visually distinct from class ownership in every view: class colour stays on the item edge, while kind colour marks the ITEMS — list rows, grid chips, dense collision stacks. On the filter row it is carried by a 7px dot and nothing else. **Narrowed 2026-08-31 by the user:** *"there is too much going on with different colors on the calendar with the meetings vs. office hours vs. hw, etc. labels at the top."* Colour on an item answers "what kind is this?"; colour on a control was decorating the control. | A reading and homework item from the same class share a class-colour edge but have different kind bands/labels in List, Week and Month. Measured on the default filter row (an empty selection means everything, so every chip is selected): 8 label colours, 8 tinted backgrounds, 8 border colours and 8 dots before — 1 / 1 / 1 and the same 8 dots after | x | | |
...
| 9.21 | **The `Times` control is an on/off switch, and a QUIET one.** The user asked for *"an on/off bubble thingy"*, and of the first attempt: *"the times toggle looks ugly."* They were right and it was not close — that version filled the track with `--accent`: 364px² of forest green at 6.82:1 against the paper, in a toolbar whose other controls say "on" at 1.07:1, and the only hue in a row of neutrals. A switch has to fill to read as on, so the answer is no COLOUR rather than less fill. It stays a switch (a MODE, where `Show past`/`Show completed` are filters that ADD rows) and it stays legible without a neighbour to compare against. | Track 22×11 filled `--muted`, knob `--panel`: 5.04:1 against the paper, down from 6.82, with no hue; knob-to-track 5.46:1, comfortably legible at 8px (`--edge` would be quieter at 3.54 but drops the knob to 3.83). Off: track `--sunk`, knob `--muted`, 2px from the left; on: 2px from the right, 10px of travel. Computed opacity 1 in both states (§3.6). 24px tall — the same as the ghost buttons beside it | x | | |
