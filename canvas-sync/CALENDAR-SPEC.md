...
| 1.10 | **A `2 days` view shows today and tomorrow against the clock**, reusing the Week view's timed geometry with two columns instead of seven. | `calView=twoday` renders `.cal-week.timed` with `--daycols:2`; `gridTemplateColumns` resolves to a gutter plus 2 tracks; at a 1200px grid each day column measures 577px against Week's 164.9px | x | | |
...
| # | Requirement | Check | Built | Live | User |
| ---: | --- | --- | :---: | :---: | :---: |
| 9.1 | A `Times` toggle in the calendar toolbar, **Week only**, persisted across reloads. The 2-day view is always timed and shows no toggle — a 2-day view that could go untimed is List with two days in it. | `#cal-times` is hidden in List, 2-day and Month and visible in Week; toggling writes `localStorage.calTimes` and survives a reload | x | | |
