...
| 3.14 | **Every dated Canvas row is its own calendar item — an AI aggregate may enrich, never swallow.** Ruled 2026-09-01: *"a lot of assignments, just regular ones in canvas like quizzes, arent showing up at all. make sure that EVERYTHING is showing up."* Mining had bundled 32 dated BUSI 380 quizzes into 8 "Session N Concept Check" items; each bundle emitted one op carrying one member's id, so 24 dated rows had no representation and 8 wore the aggregate's title. Now an aggregate reaching TWO OR MORE LIVE Canvas rows releases them: each surfaces with its exact Canvas title, own `html_url`/`submit_url`, points and due parts, inheriting the aggregate's `weight_note` and a `Part of: <aggregate>` provenance line; the aggregate itself emits nothing (double-booking is the failure the old rule guarded, and it stays guarded). Same ruling, second half: **stored titles are never truncated** — `clip()` is gone from `cleanItemTitle`/`dueTitle`/`prepTitle`/`checkpointTitle`; the two survivors (meeting and office-hours titles, SYNTHESISED strings, not names anyone typed) are disclosed in canvas-tasks.test.js/cal-names.test.js rather than silent. Display layers ellipsize; data does not. | Audit over real data: dated Canvas assignments visible as their own item 70/94 → 94/94, hidden 0; stored titles ending "…" 85 → 0; fixture worklist Sep 1 shows 7 individual S2a ops where 1 bundle stood; `.cal-collision` expands to 7 chips each with `[data-open-assignment]` and a Submit affordance | x | x | |
| 3.15 | **Coming up rows tick, and a Completed list below keeps what was ticked.** Asked 2026-09-01: *"add checkboxes here on this page for coming up assignments to be marked as completed. Add a section below for completed assignments… ordered by date the same. this way if i mismark something or i need to track smth i can still find it there. There can be vertical scroll on the page to make it fit."* The checkbox is the calendar's own (`calCheckHtml`, one document-level `[data-cal-done]` handler), so a tick here and a tick on a calendar row are the same tick; the move to Completed is immediate from `CAL_DONE`, not after the rebuild. The 8-row cap is gone: Coming up shows everything non-meeting due within 7 days and never fewer than the next 8; Completed lists every done item Coming up would list were it unticked (the literal "same span" rule had a hole when the LAST row is the one mismarked). Hidden when empty, no placeholder prose. | Fixture pane against real-data copies at 1000px: 27 rows, 27 checkboxes, 0 spacers, all 7 S2a quizzes visible, "27 due this week"; tick → Completed unhidden with 1 checked `.hu-row.is-done` (`text-decoration-line: line-through`) 0ms after the click, Coming up 26, stat 26; untick → 27/0/hidden. 16 tests in coming-up-done.test.js, 26/26 mutations killed. Narrow widths: `.hu-row` is a grid with named areas inside a `home-list` container; below 540px of section width it wraps to checkbox/day/rel/kind over a full-width title. Measured at 375px: grid, 0 of 27 rows overflow, title 287px on its own line, 24×24 checkbox, list indent 0; at 1000px the single line is unchanged (day x=39, title x=209, row 41px, 13px checkbox) | x | x | |
