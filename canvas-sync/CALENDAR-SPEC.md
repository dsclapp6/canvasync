...
| # | Requirement | Check | Built | Live | User |
| ---: | --- | --- | :---: | :---: | :---: |
| 9.1 | A `Times` toggle in the calendar toolbar, **Week only**, persisted across reloads. | `#cal-times` is hidden in List and Month and visible in Week; toggling writes `localStorage.calTimes` and survives a reload | x | | |
...
| 9.12 | **A pileup stays usable.** Two overlapping items remain side by side; a cluster becomes one count-and-time stack when three or more share the exact same slot **or when it would need more than `MAX_LANES` lanes** — four items starting together with different end times evaded the exact-slot rule and rendered as ~44px slivers. Opening it reveals every ordinary full-width chip with its checkbox, title, Submit link and drag behaviour intact. | `partitionDenseSlots` unit-tests the 2/3 boundary; the real seven BUSI 380 deadlines at Tue Sep 1 2:30 PM render as `7 due · 2:30p`, then expand to 7 chips / 7 checkboxes / 7 title buttons | x | | |
...
| 9.14 | **A block is never drawn shorter than one title line, and it occupies the minutes it is drawn for.** | `MIN_BLOCK_PX` is derived from `cal-grid.MIN_BLOCK_MIN` (55 min = 40.3px at HOUR_PX 44); measured spare below the title at the floor is +3.9px, against -4.4px at the old 32px; `layoutDay` assigns lanes on `renderedEnd`, so a 2:00 no-end deadline and a 2:30 lecture take separate lanes | x | | |
| 9.15 | **The title clamp is the number of lines that fit**, not a fixed pair of tiers. | 1 / 2 / 3 lines at `<52`, `52-66`, `>=67` — the title box starts 21.4px down and a line is 15px, so 2 lines need 51.4px and 3 need 66.4px; measured bottom spare +3.9 / +3.6 / +3.6 at 40.3 / 55 / 70px, against -14.4px at h=52 before | x | | |
| 9.16 | **No lane is narrower than the chip's own controls.** A timed chip's min-content is 84px, so at ~176px columns two lanes is the floor; a narrow lane drops the clock time, which the block's own position already states against a labelled gutter. | `MAX_LANES === 2`; at a 50% lane the chip measures `scrollWidth === clientWidth` (84/84) with the marker 5px inside the edge, against 84-in-40 and the marker 29.6px past it | x | | |
