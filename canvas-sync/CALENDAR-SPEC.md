...
| 9.21 | **The `Times` control is an on/off switch, not another ghost toggle.** The user asked for *"an on/off bubble thingy"*. It is a MODE — the days are drawn against a clock or they are not — where `Show past` and `Show completed` beside it are filters that ADD rows, so the different affordance carries a real difference. State lives in the knob's position AND the track's fill: never opacity (§3.6), and never one of the two alone, because fill alone fails anyone who cannot separate the two greens and position alone is a small target for the eye. | Off: knob 2px from the track's left, track `--sunk`, knob `--muted`. On: knob 2px from the right (12px of travel), track `--accent`, knob `--on-accent`. Computed opacity 1 in both states. The control measures 24px tall, the same as the ghost buttons beside it | x | | |
